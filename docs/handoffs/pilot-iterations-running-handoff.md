...
> Continue Ace Club LMS from `docs/handoffs/pilot-iterations-running-handoff.md`. Treat it as the single bootstrap file. Pilot V1 Phase 7 is complete and accepted for Production planning; do not repeat Phases 1–7. Read `docs/pilot-v1/evidence/phase-7-staging-acceptance-2026-08-13.md` and review `docs/pilot-v1/production-release-plan.md`. Resolve its unchecked review and read-only preflight gates, especially the unverified Production ledger state of `20260804120000_realign_weekly_course_schedule.sql`, then request a new explicit Product Owner instruction naming the authorized Production actions. Planning and read-only preflight are authorized; merging, Production SQL, Production deployment, environment changes and live-data mutation are not. Preserve staging/Production separation, Google-only controlled access, server-side authorization, RLS, release protection, Student tracker ownership, read-only Admin progress, batch-specific recordings and private file delivery.
...
- Release plan: Drafted at [conditional Production release plan](../pilot-v1/production-release-plan.md). It hard-stops if the Production ledger does not reconcile through the unverified weekly-schedule migration and defaults all authenticated smoke checks to non-mutating journeys.
...
- Exact next action: Review the conditional Production release plan and complete its read-only preflight, including reconciliation of the Production migration ledger through `20260804120000_realign_weekly_course_schedule.sql`; then obtain a separate explicit Product Owner instruction before merge, Production SQL, deployment, environment change or live-data mutation.
