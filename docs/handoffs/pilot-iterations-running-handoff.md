...
| [Phase 4 Staging Authorization Probe](../pilot-v1/phase-4-staging-authorization-probe.sql) | Ready; not run | Rollback-only Admin, Student, signed-out, inactive, pre-release, unpublished and cross-batch database checks | Run only on staging after the ordered Phase 4 migration; never include identity values or raw output in Git |
