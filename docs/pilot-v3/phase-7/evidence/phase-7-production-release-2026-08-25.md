...
## Tester attempt reset — 27 August 2026

The Product Owner authorized a Production reset control for Tanya, Unnati and Ishan while retaining the requirement that no other Student be affected. Application commit `717bf53` exposes the existing confirmed “Reset test attempt” action in Production only when the current user has an active grant for that exact Mock assignment. The delete endpoint independently verifies both current-user ownership of the attempt and an active assignment-scoped tester grant before invoking the existing service-role-only, ownership-scoped reset function. Revoking a grant removes both UI and endpoint access. Ordinary Students and Admins without a matching grant cannot use the Production reset path.
