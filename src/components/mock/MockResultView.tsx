...
import { mockUnitLabel, type MockUnit } from '@/lib/mockAttempt';
...
type ResultTab = 'overall' | MockUnit;

const RESULT_TAB_LABELS: Record<MockUnit, string> = { quant: 'QA', verbal: 'VA', data_insights: 'DI', qa: 'QA', rc: 'RC', cr: 'CR', va: 'VA', di: 'DI' };
...
export function MockResultView({ result, admin = false }: { result: Result; admin?: boolean }) {
  ...
  const resultTabs = useMemo(() => [{ id: 'overall' as const, label: 'Overall', longLabel: 'Overall' }, ...result.sections.map((section) => ({ id: section.category_key as MockUnit, label: RESULT_TAB_LABELS[section.category_key as MockUnit], longLabel: mockUnitLabel(section.category_key as MockUnit) }))], [result.sections]);
  const selectedItem = useMemo(() => result.items.find((item) => item.id === selectedItemId) ?? null, [result.items, selectedItemId]);
  const sectionItems = activeTab === 'overall' ? [] : result.items.filter((item) => item.category_key === activeTab);
  ...
  function openQuestion(item: MockResultItem, sourceId: string) {
    ...
    window.history.pushState({ ...window.history.state, mockResultQuestion: item.id, resultTab: activeTab }, '', `#question-${item.category_key}-${item.display_order}`);
  ...
  return <div className={`mock-results ${admin ? 'mock-results-admin' : ''}`}>
    ...
    <nav aria-label="Result sections" className="mock-result-tabs" role="tablist">{resultTabs.map((tab) => <button aria-selected={activeTab === tab.id} className={activeTab === tab.id ? 'active' : ''} key={tab.id} onClick={() => selectTab(tab.id)} role="tab" type="button"><strong>{tab.label}</strong><span>{tab.longLabel}</span></button>)}</nav>
