...
type Assignment = { id:string; course_id:string; release_at:string; due_at:string|null };
type Assessment = { id:string; name:string; purpose:string; status:string; draft_version:number; mock_assessment_versions?:Array<{id:string;version_number:number;mock_assessment_assignments?:Assignment[]}> };
type Item = { section:string; category_key:MockCategory; question_revision_id:string; display_order:number; stimulus_group_key?:string|null };
type Reference = { questions:Question[]; courses:Array<{id:string;name:string}> };
type Tester = { user_id:string; granted_at:string; revoked_at:string|null; profile:{full_name:string|null;email:string;role:string;is_active:boolean}|null };
...
const label=(section:MockCategory) => CATEGORY_LABELS[section];
const minutes=(section:MockCategory,count:number) => Math.round(sectionTimeSeconds(CATEGORY_PARENT[section],count)/60);
...
export function MockBuilder({initialAssessments,reference}:{initialAssessments:Assessment[];reference:Reference}) {
  ...
  const assignments=useMemo(()=>selected?.mock_assessment_versions?.flatMap(version=>version.mock_assessment_assignments??[])??[],[selected]);
  async function loadTesters(assignmentId:string){setTesterAssignmentId(assignmentId);setTesters([]);if(!assignmentId)return;try{const response=await fetch(`/api/admin/mock-builder?assignmentId=${assignmentId}`);const data=await response.json();if(!response.ok)throw new Error(data.error);setTesters(data.testers??[]);}catch(error){setMessage(error instanceof Error?error.message:'Unable to load tester access.');}}
  async function grantTester(){if(!testerAssignmentId||!testerEmail.trim())return;setBusy(true);try{const data=await call({action:'grant-tester',assignmentId:testerAssignmentId,email:testerEmail});setTesters(data.testers??[]);setTesterEmail('');setMessage(`Tester access granted to ${data.profile.email}.`);}catch(error){setMessage(error instanceof Error?error.message:'Unable to grant tester access.');}finally{setBusy(false);}}
  async function revokeTester(userId:string){if(!testerAssignmentId)return;setBusy(true);try{const data=await call({action:'revoke-tester',assignmentId:testerAssignmentId,userId});setTesters(data.testers??[]);setMessage('Tester access revoked.');}catch(error){setMessage(error instanceof Error?error.message:'Unable to revoke tester access.');}finally{setBusy(false);}}
  ...
  return <div className="mock-question-bank">
    <section className="admin-card mock-workspace-card"><div className="admin-card-header"><div><h2 className="admin-card-title">Create a mock</h2><p className="admin-page-subtitle">Build QA, RC, CR, VA, DI, mixed, or full mocks with any positive number of questions. Publishing snapshots the exact composition and proportional timing.</p></div></div><div className="mock-form-grid"><label className="mock-field"><span>Name</span><input value={name} onChange={event=>setName(event.target.value)} placeholder="April Diagnostic" /></label><label className="mock-field"><span>Purpose</span><select value={purpose} onChange={event=>setPurpose(event.target.value)}><option value="standard">Standard</option><option value="diagnostic">Diagnostic</option></select></label><button className="btn btn-primary" disabled={busy||!name.trim()} onClick={()=>void create()}>Create Draft</button></div></section>
    <section className="admin-card mock-workspace-card"><div className="admin-card-header"><div><h2 className="admin-card-title">Mock Builder</h2><p className="admin-page-subtitle">Add and reorder questions in Build, then save, review or publish from Review.</p></div><select value={selected?.id??''} onChange={event=>void choose(event.target.value)}><option value="">Select a Draft mock</option>{assessments.map(assessment=><option key={assessment.id} value={assessment.id}>{assessment.name} · {assessment.purpose} · {assessment.status}</option>)}</select></div>
      ...
      :<><div className="mock-review-summary"><h3>Review mock</h3><p>Review the selected questions and reorder them here using drag and drop or the arrow buttons. Save the partial draft at any time; publishing remains gated until the required section counts and question rules are met.</p>{sections.map(section=><span key={section} className="badge badge-upcoming">{label(section)}: {items.filter(item=>item.category_key===section).length} selected</span>)}</div>
        ...
        <div className="mock-review-summary"><h3>Pre-release tester access</h3><p>Grant an active account access only to this assignment. Tester attempts are reported separately from batch completion.</p></div><div className="mock-form-grid"><label className="mock-field"><span>Mock assignment</span><select value={testerAssignmentId} onChange={event=>void loadTesters(event.target.value)}><option value="">Choose assignment</option>{assignments.map(assignment=><option key={assignment.id} value={assignment.id}>{reference.courses.find(course=>course.id===assignment.course_id)?.name??'Batch'} · {new Date(assignment.release_at).toLocaleString('en-IN')}</option>)}</select></label><label className="mock-field"><span>Exact tester account email</span><input type="email" value={testerEmail} onChange={event=>setTesterEmail(event.target.value)} placeholder="tester@example.com" /></label><button className="btn btn-secondary" disabled={busy||!testerAssignmentId||!testerEmail.trim()} onClick={()=>void grantTester()}>Grant tester access</button><Link className="btn btn-secondary" href="/mocks">Open tester view</Link></div>{testers.filter(tester=>!tester.revoked_at).map(tester=><div className="mock-question-row" key={tester.user_id}><span><strong>{tester.profile?.full_name??'Tester'}</strong><small>{tester.profile?.email??tester.user_id} · {tester.profile?.role}</small></span><button className="btn btn-secondary btn-sm" disabled={busy} onClick={()=>void revokeTester(tester.user_id)}>Revoke</button></div>)}</>}
