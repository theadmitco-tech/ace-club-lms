...
import { assignLatestMock, assignMock, createMockAssessment, listMockAssessments, loadMockAssessment, loadMockBuilderReference, publishMock, saveMockItems, validateMock } from '@/lib/server/mockBuilder';
const headers = { 'Cache-Control': 'private, no-store' };
export async function GET(request: Request) { const auth = await requireAdmin(); if (!auth.authorized) return auth.response; try { const id = new URL(request.url).searchParams.get('assessmentId'); if (id) return NextResponse.json(await loadMockAssessment(id), { headers }); const [assessments, reference] = await Promise.all([listMockAssessments(), loadMockBuilderReference()]); return NextResponse.json({ assessments, reference }, { headers }); } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : 'Unable to load Mock Builder.' }, { status: 500, headers }); } }
