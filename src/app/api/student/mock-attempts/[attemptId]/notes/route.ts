...
export async function PUT(request: Request, { params }: { params: Promise<{ attemptId: string }> }) {
  ...
  if (typeof body?.attemptItemId !== 'string' || !note || note.length > 4000) return NextResponse.json({ error: 'Enter a note between 1 and 4,000 characters.' }, { status: 400 });
  ...
  if (error) return NextResponse.json({ error: error.code === '42501' ? 'This completed attempt is not available.' : 'Could not save this note.' }, { status: error.code === '42501' ? 403 : 422 });
