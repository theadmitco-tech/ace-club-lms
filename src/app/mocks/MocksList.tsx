  ...
  return <>
    <section className="mock-card-grid" aria-label="Released mocks">{mocks.map((mock) => {
      ...
      return <article className="mock-card" key={mock.id}><div><span className="mock-pill">{mock.tester_access ? 'Tester access' : details.purpose}</span><h2>{details.name}</h2><p>3 sections · 45 minutes each · Version {mock.mock_assessment_versions.version_number}</p>{mock.due_at && <small>Due {dueDateFormatter.format(new Date(mock.due_at))}</small>}</div>
        {mock.attempt ? <div className="mock-card-actions"><Link className="student-button" href={mock.attempt.status === 'completed' ? `/mocks/${mock.attempt.id}/results` : `/mocks/${mock.attempt.id}`}>{mock.attempt.status === 'completed' ? 'View results' : 'Resume mock'}</Link>{(mock.tester_access || allowTestReset) && <button className="mock-reset-button" onClick={() => { setError(''); setResetTarget(mock); }} type="button">Reset test attempt</button>}</div> : <button className="student-button" onClick={() => setSelected(mock)} type="button">Choose section order</button>}
      </article>;
    })}</section>
