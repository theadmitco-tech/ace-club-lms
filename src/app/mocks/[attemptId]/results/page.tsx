...
export default async function MockResultsPage({ params }: { params: Promise<{ attemptId: string }> }) {
  ...
  return <div className="student-page"><StudentHeader studentName={result.student.full_name ?? 'Student'} testerMode={identity.role === 'admin'}/><main className="student-main"><div className="student-container"><div className="mock-results-back"><Link href="/mocks">← Back to mocks</Link></div><MockResultView result={result}/></div></main></div>;
}
