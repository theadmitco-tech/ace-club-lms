...
import { formatClock, mockUnitLabel, remainingSeconds, type MockSection, type MockUnit } from '@/lib/mockAttempt';
import type { AttemptMedia, MockAttemptItem } from '@/lib/server/mockAttempts';

type SectionState = { id: string; section: MockSection; category_key: MockUnit; sequence_index: number; status: 'pending'|'active'|'review'|'submitted'|'timed_out'; deadline_at: string|null; review_edit_count: number };
type PlayerState = { attempt: { id:string; status:string; section_order:MockUnit[]; current_section_index:number; current_item_id:string|null; break_status:string; break_deadline_at:string|null; lock_version:number; mock_attempt_sections:SectionState[] }; activeSection: SectionState|null; items: MockAttemptItem[]; reviewEditedItemIds: string[]; serverNow: string };
...
export function MockPlayer({ initialState }: { initialState: PlayerState }) {
  ...
  if (state.attempt.status === 'completed') return <main className="mock-complete"><BrandLogo variant="dark" className="mock-complete-logo"/><span className="mock-success-mark">✓</span><h1>Mock completed</h1><p>Your responses were submitted successfully. Your raw results and question review are ready.</p><div className="mock-gate-actions"><Link className="student-button" href={`/mocks/${state.attempt.id}/results`}>View results</Link><Link className="student-button student-button-secondary" href="/mocks">Return to mocks</Link></div></main>;
  ...
  if (!state.activeSection || state.activeSection.status === 'pending') {
    ...
    return <main className="mock-gate"><BrandLogo variant="dark" className="mock-gate-logo"/><span className="student-eyebrow">Section {state.attempt.current_section_index + 1} of {state.attempt.section_order.length}</span><h1>{mockUnitLabel(section)}</h1><p>The timer starts only when you select Begin section.</p><ul><li>An answer counts only after you select Next and confirm Yes.</li><li>You must answer each question before continuing.</li><li>You can bookmark questions for Question Review &amp; Edit.</li><li>During review, you may edit up to 3 distinct answers.</li></ul>{error && <p className="mock-error">{error}</p>}<div className="mock-gate-actions">{canBreak && <button className="student-button student-button-secondary" disabled={busy} onClick={() => mutate('break',{action:'take'}, {refreshAfter:true})} type="button">Take 10-minute break</button>}<button className="student-button" disabled={busy} onClick={() => mutate('begin', {}, {refreshAfter:true})} type="button">Begin section</button></div></main>;
  }
  ...
  if (isReview && reviewOverview) return <div className={`mock-player mock-section-${state.activeSection.section}`}><header className="mock-player-header"><BrandLogo variant="light" className="mock-player-logo"/><div><span>{mockUnitLabel(state.activeSection.category_key ?? state.activeSection.section)}</span>{warningCopy && <span className="timer-warning-copy" role="status">{warningCopy}</span>}<strong className={warning ? 'timer-warning' : ''}>{seconds === null ? '--:--' : formatClock(seconds)}</strong></div></header>
