...
export function MockPlayer({ initialState }: { initialState: PlayerState }) {
  ...
  async function mutate(operation: string, payload: unknown = {}) {
    ...
    try {
      const response = await fetch(`/api/student/mock-attempts/${state.attempt.id}`, { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({ operation, payload, expectedLockVersion:state.attempt.lock_version, clientMutationId:crypto.randomUUID() }) });
      const result = await response.json();
      if (!response.ok) { setError(mutationErrorMessage(result.error)); if (response.status === 409) await refresh(); return false; }
  ...
  return <div className="mock-player"><header className="mock-player-header"><BrandLogo variant="light" className="mock-player-logo"/><div><span>{SECTION_LABELS[state.activeSection.section]}</span>{warningCopy && <span className="timer-warning-copy" role="status">{warningCopy}</span>}<strong className={warning ? 'timer-warning' : ''}>{seconds === null ? '--:--' : formatClock(seconds)}</strong></div></header>
    ...
    <main className="mock-player-body"><section className="mock-question-panel"><div className="mock-question-meta"><span>Question {item?.display_order} of {state.items.length}</span><button className={item?.bookmarked ? 'mock-bookmark active' : 'mock-bookmark'} disabled={!item || busy} onClick={() => item && mutate('bookmark',{attempt_item_id:item.id,bookmarked:!item.bookmarked})} type="button">{item?.bookmarked ? '★ Bookmarked' : '☆ Bookmark'}</button><span>{item?.question_snapshot.question_type}</span></div>
      <div className={item?.stimulus_snapshot?.kind === 'passage' ? 'mock-question-workspace mock-passage-workspace' : 'mock-question-workspace'}>
        ...
        {item && <div className="mock-question-response"><MockQuestionBody disabled={busy} interaction={item.response_config_snapshot.interaction} media={studentMedia(state.attempt.id, item.question_snapshot.media)} onChange={(next) => void mutate('response', { attempt_item_id:item.id, response:next, expected_response_version:responseRow?.response_version ?? 1 })} options={item.response_config_snapshot.options ?? []} response={savedResponse} responseType={item.response_config_snapshot.response_type} stem={item.question_snapshot.stem} /></div>}
      </div>
    </section><aside className="mock-navigator"><h2>Question navigator</h2><div className="mock-question-grid">{state.items.map((entry) => <button className={`${entry.id===item?.id?'current ':''}${entry.mock_responses[0]?.answered_at?'answered':''}`} key={entry.id} onClick={async()=>{if(await mutate('navigate',{attempt_item_id:entry.id})) setItemId(entry.id)}} type="button">{entry.display_order}</button>)}</div><div className="mock-review-copy"><strong>{state.activeSection.status === 'review' ? `${3-state.activeSection.review_edit_count} ${3-state.activeSection.review_edit_count === 1 ? 'edit' : 'edits'} remaining` : 'Review before submitting'}</strong><p>Bookmarked and unanswered questions remain visible here.</p></div></aside></main>
    {error && <div className="mock-toast" role="alert">{error}</div>}<footer className="mock-player-footer"><button disabled={busy || !item || item.display_order===1} onClick={() => { const previous=state.items[(item?.display_order??1)-2]; if(previous) void mutate('navigate',{attempt_item_id:previous.id}); }} type="button">Previous</button><span>Saved automatically</span>{state.activeSection.status === 'review' ? <button className="primary" disabled={busy} onClick={() => setConfirmSubmit(true)} type="button">Submit section</button> : item?.display_order === state.items.length ? <button className="primary" disabled={busy} onClick={() => mutate('review')} type="button">Review section</button> : <button className="primary" disabled={busy} onClick={() => { const next=state.items[item.display_order]; if(next) void mutate('navigate',{attempt_item_id:next.id}); }} type="button">Next</button>}</footer>
    {confirmSubmit && <div className="mock-modal-backdrop"><section className="mock-submit-dialog" role="dialog" aria-modal="true" aria-labelledby="submit-section-title"><h2 id="submit-section-title">Submit this section?</h2><p>You will not be able to return after submission. Your saved responses will be final for this section.</p><div className="mock-modal-actions"><button disabled={busy} onClick={() => setConfirmSubmit(false)} type="button">Return to review</button><button className="student-button" disabled={busy} onClick={async () => { if (await mutate('submit')) setConfirmSubmit(false); }} type="button">Confirm submission</button></div></section></div>}
  </div>;
}
